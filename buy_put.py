...
def main():
    parser = argparse.ArgumentParser(description='PUT成行買い')
    ...
    ib = IB()
    try:
        ib.connect('172.24.32.1', 4001, clientId=71)
    except Exception as e:
        log(f"接続エラー: {e}")
        return

    # PUT契約
    put = Option('QQQ', expiry, args.strike, 'P', 'SMART')
    ib.qualifyContracts(put)

    log(f"PUT ${args.strike} ({expiry}) x {args.qty}枚 成行買い")

    # 成行買い
    order = MarketOrder('BUY', args.qty)
